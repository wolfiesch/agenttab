...
AgentTab v2 is **unreleased**. The local source version is `2.0.0-rc.1`; it is not a public npm package, Chrome Web Store item, hosted site, or published release artifact. **Chrome Bridge v1.0.1 remains the available stable legacy path** until the v2 launch cutover.
...
Chrome does not expose page scripting or debugger access on browser-restricted origins such as `chrome://`, `chrome-extension://`, `devtools://`, and the Chrome Web Store. AgentTab reports these task tabs with `automation_route: "tab_only"`. Navigation, history, reload, close, load, URL, or download waits, and human-only `browser_handoff` remain available. Page snapshots, element actions, page-content waits, and raw Developer-mode CDP fail immediately with `browser_restricted_origin` and `outcome: "not_started"` before AgentTab attempts the blocked route. Use a focus-safe OS accessibility driver bound to the exact browser window when native UI work is required.

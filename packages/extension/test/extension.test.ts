...
describe("native protocol", () => {
  ...
  test("accepts selector-addressed click, type, and fill actions", () => {
    const envelope = {
      protocol: "agenttab.native",
      version: 1,
      kind: "command",
      ...
      params: {
        ...
        actions: [
          ...
          { kind: "type", selector: "input.search", text: "query" },
          { kind: "fill", selector: "input.email", text: "user@example.com" },
        ],
      },
    };
    expect(parseCommand(envelope).params).toEqual(envelope.params);

    // Both ref and selector
    expect(() => parseCommand({
      ...envelope,
      params: { ...envelope.params, actions: [{ kind: "click", ref: "e1", selector: "button" }] },
    })).toThrow("click requires exactly one of ref or selector");
    expect(() => parseCommand({
      ...envelope,
      params: { ...envelope.params, actions: [{ kind: "type", ref: "e1", selector: "input", text: "hi" }] },
    })).toThrow("type requires exactly one of ref or selector");
    expect(() => parseCommand({
      ...envelope,
      params: { ...envelope.params, actions: [{ kind: "fill", ref: "e1", selector: "input", text: "hi" }] },
    })).toThrow("fill requires exactly one of ref or selector");

    // Missing both ref and selector
    expect(() => parseCommand({
      ...envelope,
      params: { ...envelope.params, actions: [{ kind: "click" }] },
    })).toThrow("click requires exactly one of ref or selector");
    expect(() => parseCommand({
      ...envelope,
      params: { ...envelope.params, actions: [{ kind: "type", text: "hi" }] },
    })).toThrow("type requires exactly one of ref or selector");
    expect(() => parseCommand({
      ...envelope,
      params: { ...envelope.params, actions: [{ kind: "fill", text: "hi" }] },
    })).toThrow("fill requires exactly one of ref or selector");
  });
...
describe("page revision monotonicity", () => {
  ...
  test("selector actions reject ambiguous and missing element targets", async () => {
    tabStore.set(70, {
      id: 70,
      windowId: 1,
      groupId: -1,
    ...
    // Ambiguous selector (matches >1 elements)
    debuggerCommandOverride = (method) => {
      if (method === "DOM.querySelectorAll") return { nodeIds: [101, 102] };
      return undefined;
    };
    await expect(runtime.act(TASK_A, 70, 1, [{
      kind: "click",
      selector: "button.duplicate",
    }])).rejects.toMatchObject({
      code: "ambiguous_selector",
    });

    // Missing selector (matches 0 elements)
    debuggerCommandOverride = (method) => {
    ...
    await runtime.detach(70);
  });

  test("selector actions reject sensitive fields and stage consequential controls with replacement check", async () => {
    await mutateState((state) => { state.skipCommitReview = false; });
    tabStore.set(71, {
      id: 71,
      windowId: 1,
    ...
    // 1. Sensitive field via selector is rejected
    debuggerCommandOverride = (method) => {
      if (method === "DOM.querySelectorAll") return { nodeIds: [80] };
      if (method === "DOM.describeNode") return { node: { backendNodeId: 880 } };
      if (method === "Runtime.callFunctionOn") {
        return { result: { value: { agenttab_sensitive_field: true } } };
      }
      return undefined;
    };
    await expect(runtime.act(TASK_A, 71, 1, [{
      kind: "fill",
    ...
    // 2. Consequential control via selector stages for commit review
    let currentBackendNodeId = 881;
    debuggerCommandOverride = (method) => {
      if (method === "DOM.querySelectorAll") return { nodeIds: [81] };
      if (method === "DOM.describeNode") return { node: { backendNodeId: currentBackendNodeId } };
      if (method === "Runtime.callFunctionOn") {
        return {
          result: {
            value: {
    ...
    const execution = await runtime.act(TASK_A, 71, 1, [{
      kind: "click",
      selector: "button.checkout",
    }]);
    expect(execution.staged).toBeDefined();
    expect(execution.staged?.effect).toContain("Place order");
    expect(execution.staged?.preview).toMatchObject({
      kind: "click",
      selector: "button.checkout",
    });

    // 3. If target is replaced before commit, commit rejects with staged_commit_mismatch
    currentBackendNodeId = 999; // node was replaced in DOM
    await expect(runtime.commit(TASK_A, {
      native_token: execution.staged?.native_token,
    })).rejects.toMatchObject({
      code: "staged_commit_mismatch",
    });
    // Staged commit was cleaned up
    expect((await readState()).stagedCommits[String(execution.staged?.native_token)]).toBeUndefined();

    await runtime.detach(71);
  });

  test("pins resolved backend node id between describe and execution preventing replacement races", async () => {
    tabStore.set(72, {
      id: 72,
      windowId: 1,
      groupId: -1,
    ...
    let queryCount = 0;
    const executedObjectIds: string[] = [];
    debuggerCommandOverride = (method, params) => {
      if (method === "DOM.querySelectorAll") {
        queryCount++;
        // First query returns node 50; a replacement in DOM would be node 99
        return { nodeIds: [queryCount === 1 ? 50 : 99] };
      }
      if (method === "DOM.describeNode") {
        return { node: { backendNodeId: params.nodeId === 50 ? 500 : 990 } };
      }
      if (method === "DOM.resolveNode") {
        return { object: { objectId: `obj-${params.backendNodeId}` } };
      }
      if (method === "Runtime.callFunctionOn") {
        executedObjectIds.push(String(params.objectId));
        return { result: { value: { role: "button", text: "Submit" } } };
      }
      return undefined;
    };

    // 1. Selector is resolved once; same backendNodeId is classified and executed
    await runtime.act(TASK_A, 72, 1, [{
      kind: "click",
      selector: "button.submit",
    }]);
    expect(queryCount).toBe(1);
    expect(executedObjectIds).toEqual(["obj-500", "obj-500"]);

    // 2. If node is removed between describe and execution, DOM.resolveNode fails with stale_ref
    let describedOnce = false;
    debuggerCommandOverride = (method, params) => {
      if (method === "DOM.querySelectorAll") return { nodeIds: [51] };
      if (method === "DOM.describeNode") return { node: { backendNodeId: 510 } };
      if (method === "DOM.resolveNode") {
        // First resolveNode during consequence classification succeeds
        if (!describedOnce) {
          describedOnce = true;
          return { object: { objectId: "obj-510" } };
        }
        // Node removed from DOM before execution
        return {};
      }
      if (method === "Runtime.callFunctionOn") {
        return { result: { value: { role: "button", text: "Submit" } } };
    ...
    await expect(runtime.act(TASK_A, 72, 1, [{
      kind: "click",
      selector: "button.replaced",
    }])).rejects.toMatchObject({
      code: "stale_ref",
    });

    await runtime.detach(72);
  });

...
`observe` prints a compact accessibility view by default (role, accessible name, and value). Use `--role button,link`, `--name Save`, and `--limit 20` to narrow it further. Both compact and full snapshots use Chrome's real accessibility tree, so both attach Chrome's debugger. `--full` also includes node IDs, descriptions, and detailed accessibility properties. MCP callers can request `include_active_dialog=True` to receive explicit active-modal context even when role filters omit the dialog. Text extraction, HTML capture, and text waits use normal extension page access and do not attach the debugger.
...
Task sessions give an agent a set of tabs that survives extension-worker restarts and belongs only to that task. New tabs are inactive by default and placed in a named Chrome tab group. A tab opened by a task-owned tab is automatically adopted into the same session using Chrome's `openerTabId`, including links that open a new tab. Closing a session can only close tabs recorded as belonging to that session. Session records are cleared when Chrome itself restarts so stale tab numbers can never point at unrelated restored tabs.
...
Selector-based `click` uses synthesized input when Chrome can deliver it. For inactive tabs and child-frame targets it falls back to the element's click method after checking modal containment. An inactive HTTP(S) link with `target="_blank"` is opened explicitly as a background tab with its opener preserved, so popup blocking cannot orphan the new tab from its task session. `clickAt` has no selector, fallback, or child-tab ownership metadata; an inactive background tab may ignore it, so activate the tab when raw coordinates are unavoidable.

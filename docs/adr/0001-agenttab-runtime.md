...
AgentTab v2 Standard uses Chrome's required `debugger` permission and optional `scripting` permission. Current accessibility snapshots, precise click, type, and fill behavior, inactive screenshots, and task-scoped CDP helpers depend on them. Chrome rejects a manifest that lists `debugger` as optional and omits that capability, so a functional extension must declare it at install time.

The popup exposes one explicit **Enable AgentTab automation** action that requests only `scripting`. Denial leaves AgentTab installed and visibly disabled. Every debugger- or scripting-backed operation checks the current optional grant and fails closed with an enablement recovery message. Revoking `scripting` clears debugger-backed runtime state immediately and blocks future dependent operations. AgentTab MUST NOT retry the permission request outside another explicit click.

Debugger attachment MUST be lazy and limited to task-owned tabs. AgentTab reuses the task connection until its idle timeout. Standard RPC exposes no raw CDP escape hatch. The Chrome Web Store rationale MUST state this exact limited use and the install-time warning.
...
Removing the required `debugger` capability requires a separately implemented and verified non-debugger runtime. It is not part of this pivot.

The stable permission target requires `nativeMessaging`, `debugger`, `tabs`, `tabGroups`, `storage`, `alarms`, `downloads`, and `<all_urls>`, with exactly `scripting` optional. `cookies`, `history`, `bookmarks`, `contentSettings`, and `activeTab` are absent from that target. PR1 locks and statically verifies this transformation without changing or reloading the installed v1 extension.

Permission evidence follows the implementation boundary. PR1 is a decision gate: it proves the deterministic reduced-manifest transformation and platform IPC security primitives. PR2 verifies the production host lifecycle and IPC implementation. PR3 implements the optional `scripting` permission UI and lifecycle, then runs the full permission matrix three consecutive times against the exact AgentTab extension candidate on the trusted macOS runner. A failed run records the exact Chrome API call. Stable v2 cannot claim the reduced target until the matrix passes. The live probe never changes Chrome permission grants, rejects raw CDP at the non-configurable Standard adapter boundary, emits scrubbed JSONL without page data, compares active Chrome window/tab IDs rather than URLs, and runs transactional cleanup in `finally`.

...
The stable target requires `nativeMessaging`, `tabs`, `tabGroups`, `storage`, `alarms`, `downloads`, and `debugger`, with exactly `scripting` optional and `<all_urls>` host access. `cookies`, `history`, `bookmarks`, `contentSettings`, and `activeTab` are absent from that target. PR1 locks and statically verifies this transformation without changing or reloading the installed v1 extension.

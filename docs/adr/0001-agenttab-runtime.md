...
Every Standard-mode mutation MUST pass through one extension-side `prepare -> classify -> revalidate -> execute` choke point. A recognizable consequential action is staged before any side effect. Its token is bound to the task, tab, effect class, exact element fingerprint, document revision, event, preview, and a five-minute expiry. Execution revalidates the target and consumes the token exactly once.
...
AgentTab v2 Standard uses Chrome's optional `debugger` and `scripting` permissions. Current accessibility snapshots, precise click, type, and fill behavior, inactive screenshots, and task-scoped CDP helpers depend on them.

Installation MUST NOT request either capability. The popup exposes one explicit **Enable AgentTab automation** action that requests both permissions together. Denial leaves AgentTab installed and visibly disabled. Every debugger- or scripting-backed operation checks the current grant and fails closed with an enablement recovery message. Revocation clears debugger-backed runtime state immediately and blocks future dependent operations. AgentTab MUST NOT retry the permission request outside another explicit click.

Debugger attachment MUST be lazy and limited to task-owned tabs. AgentTab reuses the task connection until its idle timeout. Standard RPC exposes no raw CDP escape hatch. The Chrome Web Store rationale MUST state this exact limited use.
...
Removing the optional `debugger` capability requires a separately implemented and verified non-debugger runtime. It is not part of this pivot.

The stable permission target requires `nativeMessaging`, `tabs`, `tabGroups`, `storage`, `alarms`, `downloads`, and `<all_urls>`, with exactly `scripting` and `debugger` optional. `cookies`, `history`, `bookmarks`, `contentSettings`, and `activeTab` are absent from that target. PR1 locks and statically verifies this transformation without changing or reloading the installed v1 extension.

Permission evidence follows the implementation boundary. PR1 is a decision gate: it proves the deterministic reduced-manifest transformation and platform IPC security primitives. PR2 verifies the production host lifecycle and IPC implementation. PR3 implements the optional-permission UI and lifecycle, then runs the full permission matrix three consecutive times against the exact AgentTab extension candidate on the trusted macOS runner. The candidate may retain `activeTab` as a temporary optional contingency until those runs pass. A failed run records the exact Chrome API call before retention. Stable v2 cannot claim the reduced target until the matrix passes. The live probe never changes Chrome permission grants, rejects raw CDP at the non-configurable Standard host boundary, checks the actually focused normal window and selected tab while permitting an unfocused task window, models handoff as the sole focus transition, and polls until each task record and all of its tabs are gone. Its uploaded report contains operation names, counts, permission names, and digests only.

...
`browser_handoff` records one advisory notice per task tab and returns immediately. Notices are display metadata: they never pause the scheduler, gate another task's reads, actions, waits, updates, or finalization, and expiry only retires the reminder. A new request for the same task tab replaces the previous open notice with a new `notice_id`, so a delayed resolution cannot clear a newer notice. Task finish clears that task's notices.

The agent asks the user in its own chat, the user completes the step and reports back, and the agent verifies the page before resolving or dismissing. Multiple tasks can hold open notices at the same time and continue working.

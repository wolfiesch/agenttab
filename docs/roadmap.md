...
| T4-7 | Audit and trace export | Forwarder for JSONL, syslog, and CEF with rotation and retention policy | todo |
| T4-8 | Signed org policy bundles | Content-addressed policy bundle plus lockfile, verified before load, for managed distribution | todo |
| T4-9 | Egress allowlist | Host-side hostname and IP constraints on where an agent may drive traffic | todo |
| T4-10 | DLP hooks | Clipboard, upload, download, and screen-share chokepoints gated by policy | todo |

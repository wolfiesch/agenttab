...
| Task safety | Source implements server-bound tasks, visible groups, revisions, explicit Pause, non-blocking attention notices, default YOLO execution, and optional staged Commit records. | Controlled browser fixtures covering restart, revocation, stale revisions, handoff concurrency, and one-use Commit. |

...
- `browser_task_session_open` - create an owned task session, navigate it in the background, wait, and snapshot in one MCP call. Invalid wait arguments fail before session creation; a navigation failure closes the newly created session before returning the error

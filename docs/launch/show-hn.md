...
Two human controls are central. **Your Turn** always handles passwords, passkeys, two-factor authentication, CAPTCHA, payment secrets, and other human-only input. While a handoff is active, AgentTab applies an observation blackout for every task, so normal capture and observation calls return `needs_user`; it does not capture the person's keystrokes. Fresh state starts in unattended **Autopilot**; existing pre-policy state migrates to Strict. Review selected and Strict turn **Commit** into a best-effort barrier for recognizable sends, publishes, purchases, deletes, uploads, authorizations, and permission grants; Strict also reviews owned-tab close.

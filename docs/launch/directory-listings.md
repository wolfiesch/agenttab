...
AgentTab is a local browser runtime for AI agents. It gives each agent task-owned tabs, uses Needs your attention notices for human-only input, executes recognized consequential actions directly by default, and connects local MCP clients through per-user operating-system-native IPC.
